...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
         ...
         */
        long sum = 0L;
        sum = customerList.stream().flatMap(c -> c.getWantToBuy().stream()).count();
    ...
    public void richestCustomer() {
        ...
        Optional<Integer> richestCustomer = customerList.stream().map(e ->e.getBudget()).max(comparator);
    ...
    public void youngestCustomer() {
         ...
         * Don't use {@link Stream#sorted}
         */
        Comparator<Customer> comparator = (c1,c2) -> c1.getAge() > c2.getAge() ? 1 : -1;
